...
// Mock ToolCallBadge to keep MessageList tests simple
vi.mock("../ToolCallBadge", () => ({
  ToolCallBadge: ({ toolName, args }: { toolName: string; args: Record<string, unknown> }) => (
    <div data-testid="tool-call-badge">{`${toolName}:${args.command ?? ""}`}</div>
  ),
}));
